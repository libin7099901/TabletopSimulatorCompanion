...
function onLoad()
    ...
    -- 获取游戏名
    local game_name = Global.call("getGameName") or ""
    if game_name == "" then
        game_name = Info.Name or ""
    end
...
function handle_question(question, player)
    ...
    -- 发送请求到服务器
    WebRequest.post(tc_server_address .. "/ask", JSON.encode(request), function(response)
...
function handle_reset_session_command(args, player)
    ...
    WebRequest.post(tc_server_address .. "/session/reset", JSON.encode(request), function(response)
        if response.is_error then
            player_broadcast(player.color, "服务器错误: " .. response.error)
            return
        end
...
function handle_rulebook_command(args, player)
    ...
    elseif subcommand == "refresh_cache" then
        ...
        WebRequest.post(tc_server_address .. "/api/rulebook/refresh_rag_from_cache", JSON.encode(request), function(response)
            if response.is_error then
                player_broadcast(player.color, "服务器错误: " .. response.error)
                return
            end
...
function notify_game_loaded(game_name)
    ...
    WebRequest.post(tc_server_address .. "/api/game/loaded", JSON.encode(request), function(response)
        if response.is_error then
